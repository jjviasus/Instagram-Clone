...
struct NotificationViewModel {
    ...
    var timestampString: String? {
        ...
        return formatter.string(from: notification.timestamp.dateValue(), to: Date()) // Date() is the date right now
    }
    ...
    var notificationMessage: NSAttributedString {
        ...
        attributedText.append(NSAttributedString(string: " \(timestampString ?? "")", attributes: [.font: UIFont.systemFont(ofSize: 12), .foregroundColor: UIColor.lightGray])) // "" is our blank default value b/c it is an optional so must provide a default
